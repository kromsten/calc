...
use crate::{
    ...
    types::pair::{Pair, PopulatedPair, StoredPairType},
    ContractError,
};

use super::{
    common::{allow_implicit, denoms_from, key_from, sorted_denoms, PAIRS},
    pools::{find_pool, get_pool_pair, save_pool_pair, POOLS},
    routes::{delete_routes_with_pool, find_route, get_routed_pair, save_routed_pair, ROUTES},
};
...
fn get_pairs_full_implicit(
    ...
    if limit > 0 {
        let pools = POOLS
            .range(storage, pool_start_after, None, Order::Ascending)
            .filter_map(|pool_res| {
    ...
    if limit > 0 {
        let pools = ROUTES
            .range(storage, route_start_after, None, Order::Ascending)
