...
pub fn pool_exists(storage: &dyn Storage, denoms: &[String; 2]) -> bool {
    POOLS.has(storage, key_from(denoms))
}

pub fn get_pool(storage: &dyn Storage, key: String) -> StdResult<PopulatedPool> {
    POOLS.load(storage, key)
}

pub fn get_pool_pair(storage: &dyn Storage, key: String) -> StdResult<PopulatedPair> {
...
pub fn find_pool(storage: &dyn Storage, denoms: [String; 2]) -> StdResult<PopulatedPool> {
    ...
    if allow_implicit(storage) {
        ...
        Err(StdError::generic_err(if pool.is_ok() {
            "Direct pool pair exists but implicit pairs are not allowed"
        } else {
