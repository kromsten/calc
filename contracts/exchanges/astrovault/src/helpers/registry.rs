use cosmwasm_std::{ensure, Deps, QuerierWrapper, StdError, StdResult};

use astrovault::{
    assets::{
        asset::AssetInfo, pairs::PairInfo as StandardPoolInfo, pools::PoolInfo as StablePoolInfo,
...
use crate::{
    helpers::pool::query_assets,
    state::config::get_router_config,
    types::{config::RouterConfig, pair::PopulatedPair, pool::PoolType},
    ContractError,
};
...
fn query_ratio_pool_info(
    ...
    querier.query_wasm_smart(contract_addr, &RatioFactoryQueryMsg::Pool { asset_infos })
}
...
fn query_stable_pool_info(
    ...
    querier.query_wasm_smart(contract_addr, &StableFactoryQueryMsg::Pool { asset_infos })
}

pub fn pool_exist_in_registry(deps: Deps, pair: &PopulatedPair) -> Result<bool, ContractError> {
    ...
    let stable_assets = match pool_type {
        PoolType::Stable => query_assets(&deps.querier, factory_address, &PoolType::Stable)?
            .iter()
            .map(|a| a.info.clone())
    ...
    let pool_exists = match pool_type {
        PoolType::Ratio => {
            query_ratio_pool_info(&deps.querier, factory_address, pair.assets()).is_ok()
        }
        PoolType::Standard => {
            query_standard_pool_info(&deps.querier, factory_address, pair.assets()).is_ok()
        }
        PoolType::Stable => {
            query_stable_pool_info(&deps.querier, factory_address, stable_assets).is_ok()
        }
    };
