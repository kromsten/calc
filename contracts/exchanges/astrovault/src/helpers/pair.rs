...
use crate::types::{
    pair::{Pair, PairType, PopulatedPair, PopulatedPairType},
    pool::PopulatedPool,
};
...
impl Pair {
    ...
    pub fn pool(&self) -> Pool {
        match self.pair_type.clone() {
            PairType::Direct { address, pool_type } => Pool {
                address,
                pool_type,
    ...
    pub fn new_routed(base_asset: AssetInfo, quote_asset: AssetInfo, route: Route) -> Self {
        Pair {
            base_asset,
            quote_asset,
...
impl PopulatedPair {
    ...
    pub fn swap_msg(
        ...
        Ok(if self.is_pool_pair() {
            let pool = self.pool();
            let swap_msg =
                pool.swap_msg_cosmos(offer_asset.clone(), Some(target_asset.amount), funds)?;
            swap_msg
        } else {
            let routed_pair = if let Some(route) = route {
                let route: Route = from_json(&route)?;
                let pair =
                    Pair::new_routed(self.base_asset.clone(), self.quote_asset.clone(), route);
                validated_routed_pair(deps, &pair, Some(offer_asset.info.clone()))?
            } else {
                self.clone()
            };

            route_swap_cosmos_msg(deps, env, routed_pair, offer_asset, target_asset, funds)?
        })
    }
