use cosmwasm_std::{to_json_binary, Binary, Coin, CosmosMsg, QuerierWrapper, StdResult, Uint128};

use astrovault::{
    assets::asset::{Asset, AssetInfo},
    ratio_pool::{
        handle_msg::ExecuteMsg as RatioExecute,
        query_msg::{PoolResponse as RatioPoolResponse, QueryMsg as RatioQuery},
    },
    ...
    stable_pool::{
        ...
        query_msg::{
            PoolResponse as StablePoolResponse, QueryMsg as StableQuery,
    ...
    standard_pool::{
        ...
        query_msg::{
            PoolResponse as StandardPoolResponse, QueryMsg as StandardQuery, SimulationResponse,
        },
    },
};
...
use crate::{
    types::{
        pool::{Pool, PoolType, PopulatedPool},
...
pub fn ratio_pool_response(
    ...
    querier.query_wasm_smart(contract_addr, &RatioQuery::Pool {})
}
...
pub fn stable_pool_response(
    ...
    querier.query_wasm_smart(contract_addr, &StableQuery::Pool {})
}
...
pub fn standard_pool_response(
    ...
    querier.query_wasm_smart(contract_addr, &StandardQuery::Pool {})
}
...
pub fn query_assets(
    ...
    let assets = match pool_type {
        ...
        PoolType::Standard => standard_pool_response(querier, contract_addr)?
            .assets
            .into(),
    };
    Ok(assets)
...
impl PopulatedPool {
    ...
    pub fn combined_denoms(&self, other: &PopulatedPool) -> [String; 3] {
        if other.has_denom(&self.quote_denom()) {
            [
                self.base_denom(),
                self.quote_denom(),
                other.other_denom(&self.quote_denom()),
            ]
        } else {
            [
                self.quote_denom(),
                self.base_denom(),
                other.other_denom(&self.base_denom()),
            ]
        }
    }
    ...
    pub fn from_to_indices(&self, offer_asset: &AssetInfo) -> (u32, u32) {
        if self.quote_asset.equal(offer_asset) {
    ...
    pub fn asset_index(&self, offer_asset: &AssetInfo) -> u32 {
    ...
    pub fn other_index(&self, offer_asset: &AssetInfo) -> u32 {
        if self.base_asset.equal(offer_asset) {
            self.quote_index
        } else {
            self.base_index
    ...
    pub fn swap_simulation(
        ...
        match self.pool_type {
            ...
            PoolType::Stable => {
                let (from_index, to_index) = self.from_to_indices(&offer_asset.info);
            ...
            PoolType::Ratio => {
                ...
                let res =
                    querier.query_wasm_smart::<SwapCalcResponse>(self.address.clone(), &msg)?;
    ...
    pub fn swap_msg_binary(
        ...
        match self.pool_type {
            PoolType::Standard => to_json_binary(&StandardExecute::Swap {
                offer_asset,
                expected_return,
                belief_price: None,
                max_spread: None,
            ...
            PoolType::Stable => to_json_binary(&StableExecute::Swap {
                expected_return,
                to: None,
                swap_to_asset_index: self.other_index(&offer_asset.info),
            }),
            PoolType::Ratio => to_json_binary(&RatioExecute::Swap {
                expected_return,
                to: None,
            }),
        }
    ...
    pub fn swap_msg_cosmos(
        ...
        let swap_msg = self.swap_msg_binary(offer_asset.clone(), expected_return)?;

        if offer_asset.info.is_native_token() {
            pair_contact.execute(swap_msg, funds)
        } else {
            pair_contact.execute_cw20(offer_asset.info.to_string(), offer_asset.amount, swap_msg)
        }
    }
    ...
    pub fn astro_hop(
        ...
        let hop = match self.pool_type {
            ...
            PoolType::Stable => {
                let (from_asset_index, to_asset_index) = self.from_to_indices(offer_asset_info);

                // get full asset infos
                #[cfg(not(test))]
                let asset_infos = query_assets(querier, &self.address, &self.pool_type)?
                    .iter()
                    .map(|a| a.info.clone())
