...
use crate::{
    state::{
        config::{get_config, get_router_config},
        pairs::{find_pair, pair_exists},
    },
    types::{
        pair::{Pair, PopulatedPair, StoredPairType},
        route::{self, HopInfo, PopulatedRoute, Route, RouteHop, StoredRoute},
        wrapper::ContractWrapper,
    },
    ContractError,
};
use astrovault::assets::{
    asset::{Asset, AssetInfo},
    querier,
};
use astrovault::router::{
    handle_msg::ExecuteMsg as RouterExecute,
    state::{Hop as AstroHop, Route as AstroRoute},
};
use cosmwasm_std::{
    ensure, from_json, to_json_binary, Binary, Coin, CosmosMsg, Deps, Env, QuerierWrapper,
...
pub fn reversed(route: &Route) -> Route {
    ...
    for (index, hop) in route.iter().rev().enumerate().skip(1) {
        // get next hop in original route
        let orig_next = route.get(hops_num - index).unwrap().clone();
...
pub fn route_denoms(route: &Route) -> Vec<String> {
    route.iter().map(|h| h.denom.clone()).collect()
}

/// Return all denoms involved in the route
pub fn populated_route_denoms(route: &PopulatedRoute) -> Vec<String> {
...
pub fn route_swap_cosmos_msg(
    ...
    let astro_hops =
        route_pairs_to_astro_hops(&deps.querier, &route_pair.route(), &offer_asset.info)?;
    ...
    Ok(if offer_asset.info.is_native_token() {
        router.execute(
...
pub fn get_route_swap_simulate(
    ...
    for pool in route {
        let amount = pool.swap_simulation(&deps.querier, offer_asset.clone())?;

        let info = pool.other_asset(&offer_asset.info);

        offer_asset = Asset { info, amount };
    }
...
mod creating_routed_pairs_tests {
    ...
    use crate::{
        ...
        helpers::{
            ...
            validated::{validated_route, validated_routed_pair},
        },
        state::{
            config::{get_config, update_config, update_router_config},
            pairs::{find_pair, get_pairs, pair_exists, save_pair},
            pools::pool_exists,
            routes::route_exists,
        },
        tests::constants::{
            DCA_CONTRACT, DENOM_AARCH, DENOM_UATOM, DENOM_UNTRN, DENOM_UOSMO, DENOM_USCRT,
            DENOM_UUSDC, ROUTER_CONTRACT,
        },
        types::{
            config::Config,
            pair::{Pair, PopulatedPair, PopulatedPairType},
            pool::{PoolType, PopulatedPool},
            route::{HopInfo, PopulatedRoute, Route, RouteHop},
        },
        ContractError,
    };
    use astrovault::router::{
    ...
    use cosmwasm_std::{
        ...
        to_json_binary, Addr, Coin, ContractResult, CosmosMsg, DepsMut, Empty, Env, MemoryStorage,
    ...
    use crate::tests::common::{
        create_route, default_routed_pair, init, init_with_route, InitData,
    };
    ...
    fn can_create_router_pair() {
        ...
        assert!(route_exists(
            deps.storage,
            &[DENOM_AARCH.to_string(), DENOM_UUSDC.to_string()]
        ));
    }
    ...
    fn astrohops_work() {
        ...
        let mut pairs_hops = validated_routed_pair(deps, &pair, None).unwrap().route();

        let astro_hops =
            route_pairs_to_astro_hops(&deps.querier, &pairs_hops, &offer.info).unwrap();
        ...
        let astro_hops =
            route_pairs_to_astro_hops(&deps.querier, &pairs_hops, &target.info).unwrap();
    ...
    fn astrohops_real_work() {
        ...
        for pair in pairs {
            let pairs_hops = validated_routed_pair(deps, &pair, None).unwrap().route();

            let astro_hops =
                route_pairs_to_astro_hops(&deps.querier, &pairs_hops, &pair.base_asset).unwrap();
    ...
    fn astroroute_msg_work() {
        ...
        let route_pair = validated_routed_pair(deps, &pair, Some(offer.info.clone())).unwrap();
    ...
    fn pairs_from_initial_route_works() {
        ...
        assert_eq!(
            validated_routed_pair(deps, &pair, None).unwrap_err(),
            ContractError::RouteDuplicates {}
        );
        ...
        assert_eq!(
            validated_routed_pair(deps, &pair, None)
                .unwrap()
                .route()
                .len(),
            2
        ...
        assert_eq!(
            validated_routed_pair(deps, &pair, Some(to_asset_info(DENOM_UOSMO))).unwrap_err(),
        ...
        let validated = validated_routed_pair(deps, &pair, None).unwrap();
        ...
        assert_eq!(
            validated_routed_pair(deps, &pair, Some(to_asset_info(DENOM_USCRT))).unwrap_err(),
            ContractError::RouteNotFound {}
        );

        let reversed =
            validated_routed_pair(deps, &pair, Some(to_asset_info(DENOM_UOSMO))).unwrap();
