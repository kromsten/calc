...
use crate::{
    ...
    state::{common::key_from, pools::get_pool, routes::find_route},
    types::{
...
pub fn validated_pair_on_creation(deps: Deps, pair: &Pair) -> Result<PopulatedPair, ContractError> {
...
/// a helper validating that the pool of a direct pair is valid and convert it to new PopulatedPair
fn validated_pool_pair_on_creation(
    deps: Deps,
    pair: &Pair,
...
pub fn validated_routed_pair(
    ...
    let (base, quote, reverse) = if offer_asset.equal(&pair.base_asset) {
        (&pair.base_asset, &pair.quote_asset, false)
    } else {
        ensure!(
            offer_asset.equal(&pair.quote_asset),
            ContractError::RouteNotFound {}
        );
        (&pair.quote_asset, &pair.base_asset, true)
    };

    let from_storage = find_route(deps.storage, [base.to_string(), quote.to_string()], reverse);
...
pub fn validated_route(
    ...
    let first = route.first().unwrap();
        ...
        first.prev.asset_info.equal(base_asset),
    ...
    for (index, hop) in route.iter().enumerate().skip(1) {
        let prev_denom = &route.get(index - 1).clone().unwrap().denom;
            ...
            *prev_denom == hop.prev.asset_info.to_string(),
            ContractError::InvalidHops {}
        );
    ...
    let next = last.next.clone().unwrap();
        ...
        next.asset_info.equal(quote_asset),
        ContractError::InvalidHops {}
    );
...
fn validated_hop(deps: Deps, hop: &RouteHop, next: bool) -> Result<PopulatedPool, ContractError> {
...
fn validate_route_denoms(denoms: &Vec<String>) -> Result<(), ContractError> {
    ...
    if denoms_set.len() != denoms.len() {
        return Err(ContractError::RouteDuplicates {});
    }
    Ok(())
}
