...
impl Into<PopulatedRoute> for &PopulatedPair {
    fn into(self) -> PopulatedRoute {
        match &self.pair_type {
            PopulatedPairType::Routed { route } => route.clone(),
...
#[cfg(test)]
pub fn populated_pool(_: &QuerierWrapper, pool: &Pool) -> Result<PopulatedPool, ContractError> {
...
pub fn populated_pool(
    ...
    let assets = query_assets(querier, &pool.address, &pool.pool_type)?;
