...
use crate::helpers::{
    chain::Chain,
    helper::{instantiate_dca, instantiate_astro_standard_factory, create_astro_standard_pool, init_derivative},
};
...
fn test_instantiate_factories(chain: &mut Chain) {
    ...
    init_derivative(chain, user.account.address, "USDC".into(), "USDC".into(), 18, &user.key).unwrap();
